...
DECODE = {
    ...
    "vibethinker-3b": {
        ...
        "max_factor": 8.0,
        "task_max_factors": {
            "creative_generation": 14.0,
            "code_generation": 14.0,
        },
    },
}
...
def ollama_generate(
    ...
    reasoning, answer, truncated = (
        split_reasoning(raw) if spec["think"] else ("", raw, False)
    )
...
def _run_retention_script(model: str, script_obj: dict, max_tokens: int) -> dict:
    ...
    for i, turn in enumerate(script_obj["turns"]):
        ...
        resp = ollama_chat(
            model, messages, max_tokens=max_tokens, task="context_retention"
        )
        ...
        if turn.get("is_probe") == "mid":
            mid_probe_result = _score_probe(
                response_text, script_obj["facts"], script_obj["facts_by_mid_probe"]
            )
        elif turn.get("is_probe") == "synthesis":
            all_fact_ids = list(script_obj["facts"].keys())
            synthesis_probe_result = _score_probe(
                response_text, script_obj["facts"], all_fact_ids
            )

    weighted_score = round(
        0.3 * (mid_probe_result["score"] if mid_probe_result else 0)
        + 0.7 * (synthesis_probe_result["score"] if synthesis_probe_result else 0),
        3,
    )
...
def task_context_retention(model: str, smoke: bool = False) -> dict:
    ...
    total_stale = sum(
        r["synthesis_probe"]["stale_count"] for r in results if r["synthesis_probe"]
    )
    total_facts_checked = sum(
        r["synthesis_probe"]["total_facts"] for r in results if r["synthesis_probe"]
    )
    stale_contamination_rate = (
        round(total_stale / total_facts_checked, 3) if total_facts_checked else 0.0
    )
...
def task_summarization(model: str, smoke: bool = False) -> dict:
    ...
    for content in items:
        prompt = (
            "Summarize the following text in 3-5 sentences. Preserve the key facts.\n\n"
            + content["text"]
        )
        resp = ollama_generate(
            model, prompt, max_tokens=max_tokens, task="summarization"
        )
        ...
        results.append(
            {
                ...
                "claims_survived": sum(survived),
                "claims_total": len(content["key_claims"]),
                "claim_survival_rate": round(
                    sum(survived) / len(content["key_claims"]), 3
                ),
                "latency_s": round(resp.get("latency_s", 0), 2),
    ...
    if all(d in by_domain for d in ("prose", "mathematical", "scientific")):
        drop = round(
            by_domain["prose"]
            - (by_domain["mathematical"] + by_domain["scientific"]) / 2,
            3,
        )
    return {
        "task": "summarization",
        ...
        "mean_rouge_l": _mean(
            [r["rouge"]["rougeL"] for r in results if r.get("rouge")]
        ),
        "results": results,
    }
...
def task_open_book_qa(model: str, smoke: bool = False) -> dict:
    ...
    for item in items:
        ...
        for qa in item["questions"]:
            ...
            resp = ollama_generate(
                model, prompt, max_tokens=max_tokens, task="open_book_qa"
            )
    ...
    return {
        "task": "open_book_qa",
        "overall_accuracy": (
            round(sum(q["correct"] for q in flat) / len(flat), 3) if flat else 0.0
        ),
...
def task_structured_output(model: str, smoke: bool = False) -> dict:
    ...
    for st in items:
        resp = ollama_generate(
            model, st["prompt"], max_tokens=max_tokens, task="structured_output"
        )
    ...
    return {
        ...
        "strict_compliance_rate": round(
            sum(r["strict_compliant"] for r in results) / len(results), 3
        ),
        "lenient_compliance_rate": round(
            sum(r["lenient_compliant"] for r in results) / len(results), 3
        ),
        "results": results,
    }
...
def _v_markdown(text):
    ...
    non_md_lines = [
        l
        for l in lines
        if not (l.lstrip().startswith(("#", "-", "*", "+")) or "**" in l or "__" in l)
    ]
...
def _v_code_only(text):
    ...
    has_bt = "```" in text
    return (
        (has_def and not has_bt),
        has_def,
        {"has_function": has_def, "no_backticks": not has_bt},
    )
...
VALIDATORS = {
    "json": _v_json,
    "markdown": _v_markdown,
    "code_only": _v_code_only,
    "numbered_list": _v_numbered_list,
    "csv_line": _v_csv_line,
}
...
def task_creative_generation(model: str, smoke: bool = False) -> dict:
    ...
    for item in items:
        ...
        resp = ollama_generate(
            model, item["prompt"], max_tokens=max_tokens, task="creative_generation"
        )
        text = resp.get("text", "").strip()
        ...
        if "must_start" in item:
            checks["starts_with_required"] = (
                text.lower().lstrip("\"' ").startswith(item["must_start"])
            )
        if "acrostic" in item:
        ...
        if item.get("question_word_lines"):
            ...
            checks["all_lines_start_question_word"] = len(lines) > 0 and all(
                l.split()[0].lower() in _qwords for l in lines
            )
        results.append(
            {
                "type": item["type"],
                ...
                "constraint_pass_rate": (
                    round(sum(bool(v) for v in checks.values()) / len(checks), 3)
                    if checks
                    else None
                ),
                "latency_s": round(resp.get("latency_s", 0), 2),
    ...
    return {
        ...
        "mean_constraint_pass_rate": _mean(
            [r["constraint_pass_rate"] for r in results]
        ),
        "results": results,
    }
...
def task_code_generation(model: str, smoke: bool = False) -> dict:
    ...
    for task in items:
        ...
        resp = ollama_generate(
            model, task["prompt"], max_tokens=max_tokens, task="code_generation"
        )
    ...
    exec_only = [
        ...
        for r in results
        if r["exec_output"]
        not in (None, "static-fallback", "perl-not-installed", "ruby-not-installed")
    ]
    return {
        "task": "code_generation",
        "pass_at_1": (
            round(sum(1 for r in scored if r["passed"]) / len(scored), 3)
            if scored
            else 0.0
        ),
        "executed_pass_at_1": (
...
def judge_pass(model_results: dict, judge_model: str = JUDGE_MODEL) -> dict:
    ...
    for r in cg.get("results", []):
        s = score_1to5(
            "Rate the overall quality (creativity, coherence, fluency) of this piece.",
            r.get("text", ""),
        )
        r["judge_score"] = s
    ...
    for r in sm.get("results", []):
        s = score_1to5(
            "Rate how faithfully this summary preserves the key facts (1=poor, 5=excellent).",
            r.get("summary", ""),
        )
...
def _headline(model_results: dict) -> dict:
    ...
    return {
        ...
        "context_retention_stale_rate": t.get("context_retention", {}).get(
            "stale_contamination_rate"
        ),
        "summarization_claim_survival": t.get("summarization", {}).get(
            "claim_survival_by_domain"
        ),
        "summarization_faithfulness_drop": t.get("summarization", {}).get(
            "faithfulness_drop_prose_minus_technical"
        ),
        ...
        "structured_strict": t.get("structured_output", {}).get(
            "strict_compliance_rate"
        ),
        "structured_lenient": t.get("structured_output", {}).get(
            "lenient_compliance_rate"
        ),
        "creative_constraints": t.get("creative_generation", {}).get(
...
def _print_timing_summary(all_model_results: list):
    ...
    if not completed:
        return
    task_names = list(
        dict.fromkeys(name for r in completed for name in r.get("tasks", {}))
    )
    label_w, col_w = 24, 12
    header = (
        f"{'Model':<{label_w}}"
        + "".join(f"{n[:col_w]:>{col_w}}" for n in task_names)
        + f"{'TOTAL':>{col_w}}"
    )
    bar = "-" * len(header)
    print(f"\n{bar}\n  TIMING SUMMARY\n{bar}")
...
def run_model(
    model_name: str, smoke: bool = False, only_tasks=None, judge=False
) -> dict:
    ...
    model_results = {
        ...
        "decode_spec": decode_spec(model_name),
        "smoke": smoke,
        "tasks": {},
    }
...
def main():
    ...
    ap.add_argument(
        "--smoke", action="store_true", help="tiny token caps + fewer items, fast check"
    )
    ...
    ap.add_argument(
        "--rebuild",
        action="store_true",
        help="rebuild all_results.json + leaderboard.json from existing result files, no model runs",
    )
    args = ap.parse_args()
    if args.rebuild:
        board = rebuild_leaderboard()
